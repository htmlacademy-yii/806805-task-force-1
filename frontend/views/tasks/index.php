            ...
            <fieldset class="search-task__categories">
                ...
                <?php /* ПОЛЕ Категории с помощью ActiveForm */
                    ...
                    ->checkboxList($tasksForm->getAttributeItems('categories'), [
                        ...
                        // 'unselect' => null, // null - Не создвать скрытое поле, по умолчанию 0 - скрытое поле отправляется скрытое поле с именем value=0. Здесь ставим null, чтобы показать все задания во всех категориях. Хотя результат с 0 совпадает, зато меньше запросов в БД. Причем это нужно для корректной работы andFilterWhere, в то время как andWhere удобнее использовать скрытую отправку 0, тк не придется делать проверку на существование.
