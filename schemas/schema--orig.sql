...
CREATE TABLE IF NOT EXISTS task_runnings
(
    `id_task_failing` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `task_failing_id` INT UNSIGNED NOT NULL,
    `contractor_id`   INT UNSIGNED NOT NULL,
    PRIMARY KEY (id_task_failing),
    FOREIGN KEY (task_failing_id) REFERENCES tasks(id_task),
    FOREIGN KEY (contractor_id) REFERENCES users(id_user)
);
